...
def PlayWrapper(command):
    async def wrapper(client, message):
        ...
        command_name = message.command[0].lstrip("/").lower()
        # The Arabic `فيديو` command was previously treated as audio because
        # only the position of the letter "v" in English commands was checked.
        video = True if (
            command_name == "فيديو"
            or command_name.startswith("vplay")
            or command_name.startswith("cvplay")
            or "-v" in (message.text or "")
        ) else None

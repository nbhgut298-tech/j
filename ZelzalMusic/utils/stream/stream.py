...
def download_error(_, exc: Exception) -> AssistantErr:
    """Log the traceback and return a concise diagnostic safe for Telegram HTML."""
    LOGGER(__name__).exception("Failed to download YouTube media")
    detail = html.escape(str(exc).strip() or "No additional details")[:500]
    return AssistantErr(
        f'{_["play_14"]}\n\n<b>الاستثناء:</b> '
        f"<code>{type(exc).__name__}: {detail}</code>"
    )
